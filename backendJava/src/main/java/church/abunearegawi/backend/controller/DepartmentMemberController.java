...
public class DepartmentMemberController {
        ...
        @GetMapping
        @PreAuthorize("hasAnyRole('ADMIN', 'SECRETARY', 'CHURCH_LEADERSHIP', 'MEMBER')")
        public ResponseEntity<ApiResponse<Map<String, Object>>> getDepartmentMembers(
                ...
                List<DepartmentMemberDTO> members = departmentMemberService.findByDepartmentId(departmentId, status,
                                role);
        ...
        public ResponseEntity<ApiResponse<Map<String, Object>>> addMembersToDepartment(
                ...
                String roleInDepartment = request.get("role_in_department") != null
                                ? request.get("role_in_department").toString()
                                : "member";
        ...
        @PutMapping("/{id}")
        @PreAuthorize("hasAnyRole('ADMIN', 'SECRETARY')")
        public ResponseEntity<ApiResponse<DepartmentMemberDTO>> updateDepartmentMember(
                        @PathVariable Long departmentId,
                        @PathVariable Long id,
                        @RequestBody Map<String, Object> request) {
                DepartmentMember.Status status = request.get("status") != null
                                ? DepartmentMember.Status.valueOf(request.get("status").toString().toLowerCase())
                                : null;
                String roleInDepartment = request.get("role_in_department") != null
                                ? request.get("role_in_department").toString()
                                : null;
                ...
                DepartmentMemberDTO updated = departmentMemberService.update(id, status, roleInDepartment, notes);
