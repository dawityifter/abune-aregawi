...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    // Wrapped in a transaction: createTable + addIndex + the raw CHECK
    // constraint + the seed bulkInsert are none of them individually
    // idempotent (no IF NOT EXISTS / ON CONFLICT), so a failure partway
    // through — e.g. the bulkInsert hitting a constraint violation — would
    // otherwise leave a created-but-unseeded table that a retry of
    // `db:migrate` can't get past ("relation already exists"). No ALTER TYPE
    // is used here, so a transaction wrapper is safe.
    await queryInterface.sequelize.transaction(async (t) => {
      ...
      await queryInterface.addIndex('pledge_campaigns', ['status'], { transaction: t });
