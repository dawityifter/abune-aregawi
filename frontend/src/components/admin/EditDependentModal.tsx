...
const EditDependentModal: React.FC<EditDependentModalProps> = ({ dependent, onClose, onUpdated }) => {
  const { t } = useLanguage();
  ...
  const [form, setForm] = useState({
    ...
    baptismName: dependent.baptismName || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/members/dependents/${dependent.id}` , {
        method: 'PUT',
        headers: {
