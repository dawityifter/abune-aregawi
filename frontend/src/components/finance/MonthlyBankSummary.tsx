        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50/80 p-4">
                <div>
                    ...
                    <p className="text-sm text-gray-600">Income and expenses from bank activity, last {expanded ? 12 : 6} months</p>
                </div>
                {activeMonths.length > 6 && (
                    ...
                    >
                        {expanded ? 'Show last 6 months' : `Show last 12 months`}
                    </button>
                )}
            </div>
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-100/80">
                        <tr>
                            <th className="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Month</th>
                            <th className="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Total Income</th>
                            <th className="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Total Expense</th>
                            <th className="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Net Surplus/(Deficit)</th>
                            <th className="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Ending Balance</th>
                            <th className="px-6 py-3 text-center text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 bg-white">
                        {loading ? (
                            ...
                            <tr><td colSpan={6} className="px-6 py-6 text-center text-slate-500">No bank activity in the last 12 months. Upload a bank statement to get started.</td></tr>
                        ) : (
                                    ...
                                    <td className="whitespace-nowrap px-6 py-3 text-center">
                                        {m.pending_count === 0 ? (
                                            ...
                                            <span
                                                ...
                                                title={`${m.pending_count} of ${m.transaction_count} transactions awaiting review`}
                                            >
